...
// const fruitExists = ['abacaxi', 'pera']

const checkFruit = fruit => fruits.includes(fruit)

if (checkFruit("abacaxi")) {
  console.log('A string "abacaxi" existe no array fruits.')
}

else if (checkFruit("pera")) {
...
let msgFressPass = 'Para você, a entrada é grátis!'
let msgPayPass = 'A entrada é R$ 30,00.'

if (age  <= 7 || age >= 65) {
  console.log(msgFressPass)
} else {
  console.log(msgPayPass)
}
...
const bool = crazyArray.reduce((acc, item) => acc += typeof item === 'boolean', 0)
const num = crazyArray.reduce((acc, item) => acc += typeof item === 'number', 0)
const string = crazyArray.reduce((acc, item) => acc += typeof item === 'string', 0)
// console.log(string)

console.log(`O crazyArray tem ${bool} booleans, ${num} números e ${string} strings.`)
/*
  06
...
for(let i = 0; i < randomNumbers.length; i++) {
  if(randomNumbers[i] % 2 === 0) {
    par[i] = randomNumbers[i]
    // console.log('par: ', i)
  } else {
    impar[i] = randomNumbers[i]
    console.log('im: ', i)
  }
}

console.log(`Numeros ímpares: ${impar[0]}, ${impar[2]} e ${impar[4]}. Números pares: ${par[1]}, ${par[3]} e ${par[5]}.`)
// console.log('PAR: ', par)
