...
*/
foods = ['Coxinha', null, 'Brigadeiro']
brazilianFoods = foods.map(food => food)
...
*/
brazilianFoods = brazilianFoods.map((item) => !item ? 'Pão de Queijo' : item )
...
*/
let total = brazilianFoods.length
const foodsInfo = `Até aqui, o array "brazilianFoods" possui ${total} itens: ${brazilianFoods}`
...
*/
const newDogNames = dogNames.map((name, i) => i > 0 ? ` ${name}` : `${name}`)
...
agora é "Lilica".
*/
let countDog = dogNames.length - 1
dogNames.splice(countDog, 1)
// console.log(dogNames[countDog - 1])
...
*/
dogNames.splice(countDog, 0, "Nina")
console.log(dogNames)
const initials = dogNames.reduce((sigla, nome) => sigla += nome.charAt(), '')
...
*/
let encontre = ["co", "ca", ,"na"]
let novoNome = dogNames.reduce((acc, name, i) => {
	acc += name.match(encontre[i])
	return acc
}, '')
// console.log(novoNome)
const dessert = novoNome.replace('n', 'd')
