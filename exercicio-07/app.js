...
lionExists = animals.includes("leão");
if (lionExists) {
...
let sumRN = randomNumbers.reduce((acc, number) => {
  if (acc > 400) {
    return acc;
  }
  return (acc += parseFloat(number));
}, 0);
console.log(`A soma ultrapassou 400. Até aqui, o valor atual é ${sumRN}.`);
...
let frase = sentence.reduce(
  (acc, s) => (acc += s != "certeza" ? `${s} ` : ""),
  ""
);
console.log(frase);
...
let value = randomValues.map((v) => {
  if (strings === 3) return;

  strings += typeof v === "string" ? 1 : 0;
  bool += typeof v === "boolean" ? 1 : 0;
  total += typeof v === "boolean" ? 1 : 0;

  return { strings, bool, total };
});
